...
#if defined(NDEBUG)
#error "labrotix cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zlabrotix spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zlabrotix spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zlabrotix spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    else if (nHeight <= 75000 && nHeight > 25000) {
        nSubsidy = 2 * COIN;
    }
    else if (nHeight <= 99000 && nHeight > 75000) {
        nSubsidy = 3 * COIN;
    }
    else if (nHeight <= 125000 && nHeight > 99000) {
        nSubsidy = 2 * COIN;
    }
    else {
        nSubsidy = 3 * COIN;
...
void Misbehaving(NodeId pnode, int howmuch)
{
    ...
    if(chainActive.Height() > 600) //don't ban peers at bootstrap period
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from labrotix
...
{
    RenameThread("labrotix-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZLBOMinted()
{
...
void RecalculateZLBOSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zLBO supply
...
bool RecalculateLBOSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zlabrotix with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zlabrotixSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert LBO to zLBO
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zlabrotix spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zlabrotix serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zlabrotix mints and %d zlabrotix spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // labrotix: We use certain sporks during IBD, so check to see if they are
...
int ActiveProtocol()
{
    ...
    // SPORK_14 was used for 620020. Leave it 'ON' so they don't see > 620020 nodes. They won't react to SPORK_15
...
*/
    ...
    // SPORK_15 is used for 620021. Nodes < 620021 don't see it and still get their protocol version via SPORK_14 and their
