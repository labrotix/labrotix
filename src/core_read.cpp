...
CScript ParseScript(std::string s)
{
    ...
    static map<string, opcodetype> mapLBOames;

    if (mapLBOames.empty()) {
        for (int op = 0; op <= OP_ZEROCOINSPEND; op++) {
            // Allow OP_RESERVED to get into mapLBOames
            if (op < OP_NOP && op != OP_RESERVED)
                continue;

            const char* name = GetLBOame((opcodetype)op);
            if (strcmp(name, "OP_UNKNOWN") == 0)
                continue;
            string strName(name);
            mapLBOames[strName] = (opcodetype)op;
