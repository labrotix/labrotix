...
#include "ui_zlabrotixcontroldialog.h"
...
std::list<std::string> ZlabrotixControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZlabrotixControlDialog::listMints;

ZlabrotixControlDialog::ZlabrotixControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZlabrotixControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZlabrotixControlDialog::~ZlabrotixControlDialog()
{
    delete ui;
}

void ZlabrotixControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZlabrotixControlDialog::updateList()
{
...
void ZlabrotixControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZlabrotixControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZlabrotix_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZlabrotixControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZlabrotixControlDialog::GetSelectedMints()
{
...
void ZlabrotixControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
