...
#include "zlabrotixcontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zLBO ought to be enough for anybody." - Bill Gates, 2017
    ui->zLBOpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzLBOSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzLBO->setEnabled(false);
        ui->pushButtonMintzLBO->setToolTip(tr("zLBO is currently disabled due to maintenance."));

        ui->pushButtonSpendzLBO->setEnabled(false);
        ui->pushButtonSpendzLBO->setToolTip(tr("zLBO is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zLBOpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzLBO_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zLBO is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zLBO...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zLBO in ") +
...
void PrivacyDialog::on_pushButtonSpendzLBO_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zLBO is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zLBO
        sendzLBO();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zLBO
    sendzLBO();
}

void PrivacyDialog::on_pushButtonZlabrotixControl_clicked()
{
    ZlabrotixControlDialog* zlabrotixControl = new ZlabrotixControlDialog(this);
    zlabrotixControl->setModel(walletModel);
    zlabrotixControl->exec();
}

void PrivacyDialog::setZlabrotixControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzlabrotixSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzLBO()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid labrotix Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zLBOpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zLBO is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " LBO </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zLBO</b>";
    ...
    // use mints from zlabrotix selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZlabrotixControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZlabrotixControlDialog::GetSelectedMints();
    }
    ...
    // Clear zlabrotix selector in case it was used
    ZlabrotixControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zlabrotix Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " labrotix, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zlabrotix Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zLBO </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zLBO "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zLBO "));
    ui->labelzLBOAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzLBOSyncStatus->setVisible(fShow);
}
